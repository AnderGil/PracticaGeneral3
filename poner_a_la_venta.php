...
<html>
	...
	<body>
        ...
        <?php
            ...
            $carpetaDestino="/var/www/images/";

            # si hay algun archivo que subir
            if(isset($_FILES["file"]) && $_FILES["file"]["name"][0])
            {
            	$numImg=count($_FILES["file"]["name"]);
                ...
                for($i=0;$i<$numImg;$i++)
                {
                    ...
                    if($_FILES["file"]["type"][$i]=="image/jpeg" || $_FILES["file"]["type"][$i]=="image/pjpeg" || $_FILES["file"]["type"][$i]=="image/gif" || $_FILES["file"]["type"][$i]=="image/png")
                    {
                        ...
                        if(file_exists($carpetaDestino) || @mkdir($carpetaDestino))
                        {
                            ...
                            {
                                echo "<br>".$_FILES["file"]["name"][$i]." movido correctamente";
                            }else{
                                echo "<br>No se ha podido mover el archivo: ".$_FILES["file"]["name"][$i];
                            }
                        }else{
                            echo "<br>No se ha podido crear la carpeta: ".$carpetaDestino;
                        }
                    }else{
                        echo "<br>".$_FILES["file"]["name"][$i]." - NO es imagen jpg, png o gif";
                    }
                }
            }else{
